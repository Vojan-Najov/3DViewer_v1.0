...
static int vw_insert_vertex(vw_model_t *model, vw_point_t *v) {
	...
	if (model->vertices == NULL) {
		...
		*size_ptr = start_number_of_vertices;
		model->vertices = (vw_point_t *)malloc(sizeof(vw_point_t) * (*size_ptr));
	} else if (model->number_of_vertices >= (*size_ptr)) {
		*size_ptr *= 2;
		model->vertices = (vw_point_t *) realloc(model->vertices, 
...
int vw_reduce_vertex_array(vw_point_t **vertex_array, size_t n) {
	size_t *size_ptr;
	int err_status = 0;

	size_ptr = vw_get_vertex_array_size();

	if (*vertex_array != NULL && n < *size_ptr) {
		*vertex_array = (vw_point_t *) realloc(*vertex_array, n);
		*size_ptr = n;
