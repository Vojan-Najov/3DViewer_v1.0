...
static int vw_insert_face(vw_model_t *model,
                          unsigned int *idx, size_t number_of_idx);
...
int vw_parse_face(char *str, vw_model_t *model) {
	...
	if (number_of_idx > 0) {
		...
		if (idx == NULL) {
			...
			for (size_t i = 0; i < number_of_idx; ++i) {
...
static int vw_insert_face(vw_model_t *model,
	...
	if (model->faces == NULL) {
		...
		*size_ptr *= 2;
		model->faces = (vw_face_t *) realloc(model->faces,
                                            *size_ptr * sizeof(vw_face_t));
	}
...
int vw_reduce_face_array(vw_face_t **face_array, size_t n) {
	size_t *size_ptr;
	int err_status = 0;

	size_ptr = vw_get_face_array_size();

	if (*face_array != NULL && n < *size_ptr) {
		*face_array = (vw_face_t *) realloc(*face_array, n);
		*size_ptr = n;
		if (*face_array == NULL) {
			err_status = VW_ALLOC_ERROR;
		}
