...
//    answer = FAILURE;
  ...
  if (answer == SUCCESS && !model)
    answer = FAILURE;
  if (answer == SUCCESS && !model->vertices)
    answer = FAILURE;
  return answer;
}
...
int vw_centering_vector(vw_point_t *centering_step) {
  ...
  if (!centering_step)
    answer = FAILURE;
  if (answer == SUCCESS) {
    float xMin = model->vertices[0][0], yMin = model->vertices[0][1], zMin = model->vertices[0][2];
    float xMax = model->vertices[0][0], yMax = model->vertices[0][1], zMax = model->vertices[0][2];
    for (size_t i = 1; i < (size_t)model->number_of_vertices; i++) {
...
int vw_move(MOVE move, float a, float b, float c) {
  ...
  vw_model_t *model = NULL;
  answer = create_matrix(VW_POINT_NUMBER_OF_COORDINATES, VW_POINT_NUMBER_OF_COORDINATES, &spin);
  if (answer == SUCCESS) {
...
void vw_dim_calc(MOVE move, float *dim_ptr, matrix_t src) {
  if (move == SCALE_XYZ) {
    ...
    if (*dim_ptr < sqrtl(temp))
      *dim_ptr = sqrtl(temp);
  }
}

int vw_spin_xy(float grad) {
  return vw_move(SPIN_XY, grad, 0, 0);
}

int vw_spin_xz(float grad) {
  return vw_move(SPIN_XZ, grad, 0, 0);
}

int vw_spin_yz(float grad) {
  return vw_move(SPIN_YZ, grad, 0, 0);
}
...
int vw_create_matrix_from_vertice(matrix_t *result, vw_point_t source) {
  ...
  if (!result)
    answer = FAILURE;
  if (answer == SUCCESS && (result->columns != 1 || result->rows != 4 || !result->matrix))
    answer = FAILURE;
  if (answer == SUCCESS) {
...
int vw_create_matrix_from_move(matrix_t *result, MOVE move, float a, float b, float c) {
  int answer = SUCCESS;
  if (!result)
    answer = FAILURE;
  if (answer == SUCCESS && (result->columns != 4 || result->rows != 4 || !result->matrix))
    answer = SUCCESS;
  if (answer == SUCCESS) {
...
int vw_copy_vertices_data(matrix_t *source, vw_point_t *dst) {
  ...
  if (!source || !dst)
    answer = FAILURE;
  if (answer == SUCCESS && (source->columns != 1 || source->rows != VW_POINT_NUMBER_OF_COORDINATES || !source->matrix)) {
    answer = FAILURE;
  }
  if (answer == SUCCESS) {
    for(int i = 0; i < VW_POINT_NUMBER_OF_COORDINATES; i++) {
